...
import com.magicalhag.autohag.auto.games.arknights.misc.arknightsCredits
import com.magicalhag.autohag.auto.games.arknights.misc.arknightsRewards
...
suspend fun AutoService.arknightsBase(text: Text, state: ArknightsState, onCompletion: () -> Unit) {
    when (state.baseTask) {
        ...
        "CC" -> arknightsBaseFacility(text, "CC", state) {
            state.facilityAvailableOpNames = mutableListOf()
            state.facilityBlacklistedOpNames = mutableListOf()
            state.facilityAlreadySelectedOpNames = mutableListOf()
            state.baseTask = "REALIGN1"
        }
        ...
        "TP3" -> arknightsBaseFacility(text, "TP3", state) {
            state.facilityAvailableOpNames = mutableListOf()
            state.facilityBlacklistedOpNames = mutableListOf()
            state.facilityAlreadySelectedOpNames = mutableListOf()
            state.baseTask = "REALIGN4"
        }
        ...
        "FAC3" -> arknightsBaseFacility(text, "FAC3", state) {
            state.facilityAvailableOpNames = mutableListOf()
            state.facilityBlacklistedOpNames = mutableListOf()
            state.facilityAlreadySelectedOpNames = mutableListOf()
            state.baseTask = "REALIGN7"
        }
        ...
        "DORM4" -> arknightsBaseFacilityDorm(text, "DORM4", state) {
            state.facilityAlreadySelectedOpNames = mutableListOf()
            onCompletion()
        }
    }
